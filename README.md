...
- **Collected**: Users deposit CKB into liquidity buckets and receive iCKB.
- **Maintained**: Liquidity buckets accrue interest in the NervosDAO.
- **Released**: If a user wants to exchange iCKB for CKB, they can use any liquidity bucket that is at maturity.
...
Naturally the user can choose to deposit an integer multiple of the fixed deposit, the protocol takes care of splitting it in many fixed NervosDAO deposits and mints the correct iCKB amount.
...
- The iCKB holder **freely** chooses from the pool a deposit to withdraw from. All deposits are of a fixed iCKB-equivalent size, the only differentiating factor between them is the recurring maturity date.
- With the first transaction the user sends to the protocol the fixed amount of iCKB and chooses the specific deposit to withdraw from, while the protocol in turn assigns to the user that specific deposit and burns the received iCKB.
...
iCKB is an on-going research project:
